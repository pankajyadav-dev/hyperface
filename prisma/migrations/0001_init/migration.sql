-- =====================================================================
--  0001_init — Hyperface Meeting Room Booking
--  Hand-written so we can include DB-level guarantees Prisma can't model:
--   * anti double-booking (EXCLUDE USING gist)
--   * capacity / attendee / time-order CHECKs
--   * generated tsrange column used by the overlap constraint
--  Applied verbatim by `prisma migrate deploy`.
-- =====================================================================

-- Needed to combine equality (=) with range overlap (&&) in one
...
-- Room name unique per floor (Floor 3 "Neptune" != Floor 5 "Neptune").
...
CREATE TABLE "bookings" (
    ...
    -- Generated half-open range [start, end) for the overlap guard.
    "during" tsrange GENERATED ALWAYS AS (
        tsrange(("booking_date" + "start_time"), ("booking_date" + "end_time"), '[)')
    ) STORED,

    -- THE rule: no two *booked* rows for the same room may overlap in
    -- time. Cancelled rows are excluded, so cancelling frees the slot.
...
-- --- auto-touch updated_at (protects non-Prisma writers too) ----------
