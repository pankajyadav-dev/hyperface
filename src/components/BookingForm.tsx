...
export function BookingForm({ rooms, employees, today }: Props) {
  ...
  const run = (action: typeof createBooking) =>
    startTransition(async () => {
      const result = await action(state ?? { ok: false }, buildFormData());
      setState(result);
    });
    ...
    <div>
      {noRooms && (
        <div className="alert alert-warn">
          No rooms yet. <a href="/rooms">Add a room</a> first.
        </div>
      )}
      {noEmployees && (
        <div className="alert alert-warn">
          No employees yet. <a href="/employees">Register an employee</a> first.
        </div>
      )}
